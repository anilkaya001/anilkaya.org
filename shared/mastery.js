...
export function intervalForLevel(value) {
...
export function applyMastery(record, options) {
...
export const apply = applyMastery;
export const schedule = applyMastery;
...
export function dueCount(bank, mastery, progress, today) {
  return reviewCandidates(bank, mastery, progress, today).filter((candidate) => candidate.due).length;
}

export function selectSession(bank, mastery, progress, today, limit = 5) {
  requireDay(today);
...
export const MasteryScheduler = Object.freeze({
  LEVEL_MIN,
  LEVEL_MAX,
  INTERVAL_DAYS,
  normalizeDay,
